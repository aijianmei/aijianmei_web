...
class PagerApi extends Api {
    ...
    function makepg($i,$pg)
        {
        if ($i==$pg){
            return $this->style['current']?" <a class='".$this->style['current']."'>".$i."</a>":" <a %s>".$i."</a>";
        }else{
            return $this->style['num'] ? " <a href=".PagerApi::replacepg($this->url,5,$i)." class='".$this->style['num']."'>".$i."</a>" :" <a href=".PagerApi::replacepg($this->url,5,$i)." >".$i."</a>";
        }
    }
    ...
    function gotoback($pg)
        {
        if ($pg-1>0){
            return $this->style['pre']?$this->gotoback="<a href=".$this->replacepg($this->url,2,0)." class='".$this->style['pre']."'>上一页</a>":$this->gotoback="<a href=".$this->replacepg($this->url,2,0)." %s>上一页</a>";
        }else{
            return $this->style['pre']?$this->gotoback="<a class='".$this->style['pre']."'>上一页</a> ":$this->gotoback="<a %s>上一页</a> ";
        }
    }
    ...
    function gotonext($pg,$page)
        {
        if ($pg < $page){
            return $this->style['next']?" <a href=".$this->replacepg($this->url,1,0)." class='".$this->style['next']."'>下一页</a>":" <a href=".$this->replacepg($this->url,1,0)." %s>下一页</a>";
        }else{
            return $this->style['next']?" <a class='".$this->style['next']."'>下一页</a>":" <a %s>下一页</a>";
        }
    }
