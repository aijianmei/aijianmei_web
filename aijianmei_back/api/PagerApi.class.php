...
class PagerApi extends Api {
	...
	function PagerApi($countall=0, $countlist=20, $style="page"){
	...
	function makepg($i,$pg){
		if ($i==$pg){
			...
			return " <a href=".PagerApi::replacepg($this->url,5,$i)." class='".$this->style."'><u>".$i."</u></a>";
		}
	}
	//生成上一页等信息的函数
	function gotoback($pg){
		if ($pg-1>0){
			return $this->gotoback=" <a href=".$this->replacepg($this->url,3,0)." class='".$this->style."'>首页</a> <a href=".$this->replacepg($this->url,2,0)." class='".$this->style."'>上一页</a>";
		}else{
	...
	function gotonext($pg,$page){
		if ($pg < $page){
			return " <a href=".$this->replacepg($this->url,1,0)." class='".$this->style."'>下一页</a> <a href=".$this->replacepg($this->url,4,0)." class='".$this->style."'>尾页</a>";
		}else{
