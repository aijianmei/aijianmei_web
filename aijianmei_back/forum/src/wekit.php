 ...
 * @version $Id: wekit.php 28954 2013-05-31 06:56:09Z yetianshi $
 * @package wekit
 */
class Wekit {
	...
	public static function run($name = 'phpwind', $components = array()) {
		...
		if($_REQUEST['ai_pwreg']==1||$_REQUEST['ai_pwlogin']==1){
			$_SESSION['ai_pwlogin_allow']=1; 
			$_SESSION['ai_pwapicode']=$_REQUEST['ai_pwreg']==1?'ai_pwreg':'';
			$_SESSION['ai_pwapicode']=$_REQUEST['ai_pwlogin']==1?'ai_pwlogin':'';
			WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token'));
		}
		$application->run();
	}
