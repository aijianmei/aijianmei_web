...
var init = function(){
    ...
    var choice_height_this = function(e){
		...
		if(_target.innerHTML=='小于140cm'||_target.innerHTML=='大于或等于200cm'){
			if(!default_height){
				$("#dt_height_edit_div").hide();
				$(".dt_height_input").show();
    ...
    var choice_weight_this = function(e){
		...
		if(_target.innerHTML=='小于40kg'||_target.innerHTML=='大于或等于100kg'){
			if(!default_weight){
				...
				$("#dt_weight_edit_div").hide();
				$(".dt_weight_input").show();
    ...
    tag_id.onblur = function(){
		...
		$(".dt_sure_remove").click(function(){
			var delstr=$(this).parent().find('.dt_sure_content').html();
			$("#keywordlist").find('input[value='+delstr+']').remove();
			$(this).parent().remove();
		});
...
init(); 
//kontem jq part for delete keyword tags list input values
$(".dt_sure_remove").click(function(){
	var delstr=$(this).parent().find('.dt_sure_content').html();
	$("#keywordlist").find('input[value='+delstr+']').remove();
	$(this).parent().remove();
});
$(document).keydown(function(event) {//回车事件触发onblur
if (event.keyCode == 13) {
...
$('#tag').dblclick(function() {//双击触发onblur
	//alert(1);
	$('#tag').blur();
});

function changDtDiv(key){
	var keystr=$("#dt_"+key+"_write").val();
	var activename=document.activeElement.id; // 当前获取焦点的对象
	
	if(keystr==''&&activename!=keystr)
	{
		$(".dt_"+key+"_input").hide();
		$(".dt_"+key+"_change").click();
	}else{
		switch(key){
			case 'weight':
			var	vstr=keystr+'kg';break;
			case 'height':
			var	vstr=keystr+'cm';break;
			case 'year':
			var	vstr=keystr+'岁';break;
		}
		$(".dt_"+key+"_finish").html(vstr);
		$("#dt_"+key+"_finish").val(vstr);
		$("#dt_"+key+"_edit_div").show();
		$(".dt_"+key+"_input").hide();
	}
}
