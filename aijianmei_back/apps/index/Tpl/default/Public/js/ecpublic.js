...
jq(function(){
    ...
    //点击切换
    jq("#banner .ul_2 li").click(function(){
        index = jq("#banner .ul_2 li").index(this);
        move(index);
    })
    ...
    var move = function(index){
        ...
        jq("#banner .ul_1").stop(true,false).animate({"left":nowleft},300);
        jq("#banner .ul_2 li").css("border","3px solid transparent").eq(index).css("border","3px solid #4298CE");
...
jq(function(){
    jq(".login").click(function(e){
        ...
        jq("div.body").slideDown(300,function(){
            jq("html").css("overflow","hidden").height("100%");
            jq(this).css({"display":"block","opacity":"0.7"});
            jq("div.sheet").slideDown(200);
            jq("div.sheet").css("display","block");
        });
    });
    jq(".close_btn").click(function(){
        jq("html").css("overflow","scroll");
        jq("div.sheet").slideUp(200,function(){
        jq("div.sheet").css("display","none");
        jq("div.body").slideUp(300,function(){
            jq("div.body").css("display","none");
        });
        
        });
    });
    
    jq(".text_input input").focus(function(){
        jq(this).siblings().hide();          
    });
    jq(".text_input input").blur(function(){
        if(!(jq(this).val())){
            jq(this).siblings("label").show();
        }
        else{
            var e_reg = new RegExp(),
                p_reg = new RegExp();
                e_reg = /^\w+((_-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|_-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+jq/,
                p_reg = /[0-9A-Za-z]{6,16}/;
            var email = jq("#mail").val(),
                psd = jq("#psd").val();
    ...
    jq(".more").mouseover(function(){
        ...
        jq(this).mouseout(function(){
            jq(this).children(jq("a")).first().removeClass("on");
            jq(".more>ul").hide();
        })      
    })
    //导航栏样�?
...
// //视频列表 切换分类
// jq("li.select>a").mouseover(function(){
//  jq(this).addClass("pre").siblings().removeClass("pre");
    
// })   
//鼠标移上图片显示进入
jq("li .show_enter").add("div .show_enter").mouseover(function(){
    jq(this).children(".enter_icon").css("display","block");
    jq(this).children("img").css("border-color","#21ace3");
    jq(this).children().children(".v_enter").css('background','url(images/wm3.png) no-repeat 0 -490px')
    jq(this).mouseout(function(){
        jq(this).children(".enter_icon").css("display","none");
        jq(this).children("img").css("border-color","transparent");
...
//添加目录动�?�?--------------------------------------
           jq('.nav_cf').mouseover(function(){
                ...
                jq('.nav_detail').css('opacity','0').eq(index).css('opacity','1');
                jq('.bg_opacity').css('opacity','0.4').eq(index).css('opacity','0');
           })
           jq('.nav_cf').mouseout(function(){
                jq('.nav_detail').css('opacity','0');
                jq('.title_hint').css('opacity','1');
           })
           jq('.position_nav').mouseleave(function(){
                jq('.bg_opacity').css({'opacity':'0'})
           })
//为板块添加css3渐变边框---------------------------
        jq(".tr_classify").add(".recommend").add(".tr_top").hover(
            function(){
                jq(this).css("box-shadow","0 0 8px #21ace3")
            },
            function(){
