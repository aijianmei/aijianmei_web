<?php
class LifestyleAction extends Action {
...
";
    ...
    $bannerinfo=$bannerinfo['lifestyle'];
    ...
    /*$bannerinfo=array(
		'1'=>array(
			'name'=>'美味七道菜，健康炒出来',
			'img'=>'../Public/images/banner/lifestyle_1.jpg',
			'url'=>'/index-Index-articleDetail-103.html'
			),
		'2'=>array(
			'name'=>'5招秘诀！新陈代谢瘦起来！',
			'img'=>'../Public/images/banner/lifestyle_2.jpg',
			'url'=>"/index-Index-articleDetail-104.html"),
		'3'=>array(
			'name'=>'快速减压，12招必备',
			'img'=>'../Public/images/banner/lifestyle_3.jpg',
			'url'=>"/index-Index-articleDetail-106.html"),
		'4'=>array(
			'name'=>'跑步小贴士！常见损伤“防与治”',
			'img'=>'../Public/images/banner/lifestyle_4.jpg',
			'url'=>"/index-Index-articleDetail-107.html"),
		);*/
    ...
    public function index()
    {
        ...
        //assign hotArticles
		$orderTableSql="SELECT a.* FROM ai_article_category_group a, ai_article_category c WHERE a.category_id = c.id AND c.channel ='".$map['channel']."'";
		...
		$pagerData=$this->pageHtml($countInfo[0]['cnums'],20,$pglimit,'/index.php?app=index&mod=Lifestyle&act=index&ctype=2&pg=');
		$pagerArray = $pagerData['html'];
		
		$order = 'reader_count';
		//$hotArticles = D('Article')->getTrainArticles($order);
        $hotArticles = D('Article')->getArticlesListType($order,'',($pglimit-1)*20,$nums,$map['channel']);
        //print_r( $hotArticles);
		$this->assign('hotArticlespage', $pagerArray);
        $this->assign('hotArticles', $hotArticles);
        
		
		$pagerData=$this->pageHtml($countInfo[0]['cnums'],20,$pglimit,'/index.php?app=index&mod=Lifestyle&act=index&ctype=1&pg=');
		$pagerArray = $pagerData['html'];
        //assign lastArticles		
        $order = 'create_time';
		//$lastArticles = D('Article')->getTrainArticles($order);
        $lastArticles = D('Article')->getArticlesListType($order,'',($pglimit-1)*20,$nums,$map['channel']);
        ...
        $this->assign('headertitle', '生活方式');
		 //header current add by kon at 20130415
		$this->assign('_current', 'lifestyle');
