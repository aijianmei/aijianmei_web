...
class ShopService extends Service {
	...
	public function register($username, $email, $password){
		...
		$id = M('')->query('select user_id from ecs_users where user_name="'.$username.'"');
		
		$this->_login($id[0]['user_id']);
	}
	// 登录商城
	public function login($id){
		$user = M('user')->where(array('uid'=>$id))->find();
		$uid = M('')->query('select user_id from ecs_users where user_name="'.$user['uname'].'"');
		
		if(!$uid[0]['user_id']) {
			$this->register($user['uname'], $user['email'], $user['password']);	
		}else {
			$this->_login($uid[0]['user_id']);
		}		
	}
	// 登出商城
